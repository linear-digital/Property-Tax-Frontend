...
const PaymentReceiptDowload = ({ payment }: { payment: any }) => {
    ...
    const formData: any = {
        branch: branch,
        payment: payment,
        invoiceNumber: payment?.invoice_id?.invoice_id,
    ...
    return (
        ...
        >
            {({ loading }) =>
                loading ? 'Generating PDF' : <span className='dark:text-white text-dark'><FontAwesomeIcon icon={faFilePdf} /> Download Payment Receipt</span>
            }
        </PDFDownloadLink>
    );
};
